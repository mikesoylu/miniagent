...
The complete script, including its embedded Bash/awk JSON fallback, is **25.70 KiB gzipped** (26,321 bytes), measured with `gzip -n -c miniagent.sh | wc -c` using default compression.
...
- Keeps tool progress on stderr so normal and JSON answers on stdout are easy to pipe. Model, reasoning, context usage, and turn count print once below the final answer or cancellation message, rather than before every model call.
